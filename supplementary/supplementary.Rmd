...
expl_visual$exp_mods_time$mods$fit_exp_block %>% 
  ...
  customize_table(caption = "Results of exploratory LMMs investigating the effect of block (1 and 2) group (CG and UG) and Cue (Negative and Neutral) on Expectancy rating in Experiment 1. We reported the unstandardized regression coefficients, standard errors (SE), 95% confidence intervals (CI), and the associated t-test.")
```

```{r anova-visual-block}
expl_visual$exp_mods_time$mods$fit_exp_block %>% 
  tidy_anova() %>% 
  mutate(mod = "fit_exp") %>% 
  prep_names_anova() %>% 
  anova_table(single_model = TRUE) %>% 
  customize_table(caption = "Anova table of exploratory LMM investigating the effect of block (first vs second part of the experiment) group (CG and UG) and Cue (Negative and Neutral) on Expectancy rating in Experiment 1.")
```

```{r plot-visual-block, fig.cap="Box-plot of expectancy ratings in Experiment 1 according to the group (CG vs. UG), and the cue S1 color (cue-neg vs. cue-neu). Points represent the mean estimated value for each participant and condition."}
...
expl_visual$exp_mods_time$mods$fit_exp_block %>% 
  ...
  ggplot(aes(x = s1_color, y = .mean, fill = group)) +
...
```{r mod-auditory-block}
expl_auditory$exp_mods_time$mods$fit_exp_block %>% 
  tidy_fit() %>% 
  mutate(mod = "fit_exp") %>% 
  prep_names_model() %>% 
  model_table(single_model = TRUE) %>% 
  customize_table(caption = "Results of exploratory LMMs investigating the effect of trial (mean centered) on expectancy ratings in Experiment 2. For each model, we reported the unstandardized regression coefficients, standard errors (SE), 95% confidence intervals (CI), and the associated t-test.")
```

```{r anova-auditory-block}
expl_visual$exp_mods_time$mods$fit_exp_block %>% 
  tidy_anova() %>% 
  mutate(mod = "fit_exp") %>% 
  prep_names_anova() %>% 
  anova_table(single_model = TRUE) %>% 
  customize_table(caption = "Anova table of exploratory LMM investigating the effect of block (first vs second part of the experiment) group (CG and UG) and Cue (Negative and Neutral) on Expectancy rating in Experiment 2.")
```

```{r plot-auditory-block, fig.cap="Box-plot of expectancy ratings in Experiment 2 according to the group (CG vs. UG), and the cue S1 color (cue-neg vs. cue-neu). Points represent the mean estimated value for each participant and condition."}
expl_auditory$exp_mods_time$mods$fit_exp_block %>% 
  get_effects(y = exprating, workerId, group, block, s1_color) %>% 
  mutate(resp = case_when(resp == tolower("Exprating") ~ "Expectancy",
                          resp == tolower("Valrating") ~ "Valence",
